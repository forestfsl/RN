...
class AnimatedGradientTransition extends _react.Component {
  constructor(props) {
    super(props);
    this.state = {
      colors: props.colors,
      prevColors: props.colors,
      animatedColors: props.colors.map(() => new _reactNative.Animated.Value(0))
    };
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const mutableProps = _lodash.default.pick(nextProps, keys);

    const stateToCompare = _lodash.default.pick(prevState, keys);
    ...
    if (!_lodash.default.isEqual(mutableProps, stateToCompare)) {
  ...
  static animateGradientTransition(animatedColors, curColors, prevColors, animation) {
    ...
    if (!_lodash.default.isEqual(prevColors, curColors)) {
      // Update number of animatedValue if the length is different
      if (curColors && animatedColors.length !== curColors.length) {
        animatedColors = curColors.map(() => new _reactNative.Animated.Value(0));
      } else {
        animatedColors.forEach(animatedColor => animatedColor.setValue(0));
      } // Parallel animation of all background colors


      _reactNative.Animated.parallel(animatedColors.map(animatedColor => {
        return _reactNative.Animated.timing(animatedColor, {
  ...
  _generateColorsProps(interpolatedColors) {
    ...
    interpolatedColors.forEach((interpolateColor, index) => {
      ...
      props = _lodash.default.merge(props, {
  ...
  render() {
    ...
    const otherProps = _lodash.default.omit(props, 'animation');

    return /*#__PURE__*/_react.default.createElement(_AnimatedLinearGradient.default, _extends({}, otherProps, animatedColorsProps), /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, children));
  }
...
var _default = AnimatedGradientTransition;
